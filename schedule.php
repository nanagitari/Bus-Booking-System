						...
						resp = JSON.parse(resp)
							if(Object.keys(resp).length > 0){
								...
								Object.keys(resp).map(k=>{
									var tr = $('<tr style="align-items:center"></tr>');
									tr.append('<td class="text-center">'+(i++)+'</td>')
									tr.append('<td class="">'+resp[k].date+'</td>')
									tr.append('<td class="">'+resp[k].bus+'</td>')
									tr.append('<td class=""> <span style="font-weight:bold">FROM:</span> '+resp[k].from_location+'<br> <span style="font-weight:bold">TO:</span> '+resp[k].to_location+'</td>')
